...
class MLPGaussianActor(Actor):
    ...
    def __init__(self, feature_layer, obs_dim_self, obs_dim_target, act_dim,
                 mis_num, alert_num, hidden_sizes, rnn_hidden_shape, activation, device):
        super().__init__()
        self.device = device
        # attribute
        ...
        self.alert_flatten = nn.Flatten(1, -1).to(device)  # out: (batch_size, alert_num*2)

        if feature_layer is None:
            self.mu_net = mlp([obs_dim_self + obs_dim_target + 2 * alert_num + mis_num] +
    ...
    def get_forward(self, obs_self, obs_target, missile_states, missile_alerts, hidden=None, seq_len=1):
        ...
        if self.feature_layer is None:
            if len(missile_alerts.size()) < 3:
                mis_state = missile_states.unsqueeze(0)  # (1, mis_num)
                mis_alert = missile_alerts.unsqueeze(0)  # (1, alert_num, 2)
            else:
                mis_state = missile_states  # (batch_size, mis_num)
                mis_alert = missile_alerts  # (batch_size, alert_num, 2)

            mis_alert = self.alert_flatten(mis_alert)  # (batch_size, 2*alert_num)
...
class MLPCritic(nn.Module):
    ...
    def __init__(self, feature_layer, obs_dim_self, obs_dim_target,
                 mis_num, alert_num,
        ...
        self.net = mlp([obs_dim_self + obs_dim_target + 2 * alert_num + mis_num]
...
class ActorTwoCriticAlone(nn.Module):
    ...
    def __init__(self, obs_dim_self, obs_dim_target, act_dim, mis_num, alert_num, hidden_shape=(1, 128),
        ...
        if feature_layer is None:
            # actor model
            self.actor = MLPGaussianActor(feature_layer, obs_dim_self, obs_dim_target, act_dim,
                                          mis_num, alert_num, hidden_sizes, hidden_shape, activation,
                                          device).to(device)
        ...
        self.v_task = MLPCritic(feature_layer, obs_dim_self, obs_dim_target,
                                mis_num, alert_num,
                                hidden_sizes, activation, device).to(device)

        # build value function for network distillation
        self.v_rnd = MLPCritic(feature_layer, obs_dim_self, obs_dim_target,
                               mis_num, alert_num,
                               hidden_sizes, activation, device).to(device)
